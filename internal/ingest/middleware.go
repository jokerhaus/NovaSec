...
func (s *Server) rateLimitMiddleware() gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		// Проверяем лимит
		if info.Count >= s.config.RateLimit.RequestsPerMinute {
			info.Blocked = true
			info.BlockUntil = time.Now().Add(s.config.RateLimit.BlockDuration)
			mu.Unlock()
			
			c.JSON(http.StatusTooManyRequests, gin.H{
				"error": "Rate limit exceeded",
				"retry_after": s.config.RateLimit.BlockDuration.Seconds(),
			})
			c.Abort()
			return
		}
		...
		c.Header("X-RateLimit-Limit", fmt.Sprintf("%d", s.config.RateLimit.RequestsPerMinute))
		c.Header("X-RateLimit-Remaining", fmt.Sprintf("%d", s.config.RateLimit.RequestsPerMinute-info.Count))
...
func (s *Server) bodySizeMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		if c.Request.ContentLength > s.config.BodySizeLimit {
			c.JSON(http.StatusRequestEntityTooLarge, gin.H{
				"error": "Request body too large",
				"max_size": s.config.BodySizeLimit,
